...
class AriaGUI(ctk.CTk):
    ...
    def toggle_listening(self):
        ...
        if self.is_listening:
            self.toggle_btn.configure(text="■ Stop", fg_color="#8c2b2b")
            self.set_status("Listening", "#ff9800")
            self.msg_queue.put({"type": "trigger_listen"})
        else:
            self.toggle_btn.configure(text="● Start", fg_color="#2b5a8c")
            self.set_status("Idle", "#4caf50")
    ...
    def _listen_and_respond(self, listener, stt, brain, conv, registry, tts, config):
        ...
        if result["tool_call"]:
            tc = result["tool_call"]
            tool_result = registry.execute(tc["tool"], tc.get("args", {}))
            conv.add_tool_result(tc["tool"], tool_result)
    ...
    def _aria_loop(self):
        ...
        while self.running:
            try:
                if self.is_listening:
                    self._listen_and_respond(listener, stt, brain, conv, registry, tts, config)
                    if self.is_listening:
                        self.msg_queue.put({"type": "status", "text": "Listening...", "color": "#ff9800"})
                    continue

                self.msg_queue.put({"type": "status", "text": "Clap or say 'Aria'...", "color": "#4caf50"})
                wake.wait_for_wake()
